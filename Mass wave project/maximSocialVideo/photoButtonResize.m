...
*/
@implementation photoButtonResize
@synthesize dataImage;

@synthesize activityIndicator;
@synthesize videoUserId;
@synthesize boolResize;
...
//        
//        
//        imgOnBtn.image = tempImg2;
        
        
        imgOnBtn.frame = CGRectMake(imgOnBtn.frame.origin.x, imgOnBtn.frame.origin.y, tempImg.size.width, tempImg.size.height);
        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, tempImg.size.width, tempImg.size.height);
        
        imgOnBtn.image = tempImg;
        ...
        if ([self.dataImage writeToFile:path atomically:YES] != FALSE) {
            // return TRUE;
        } else {
            //return FALSE; // Error
        }
    ...
    else
    {
        UIImage *tempImg3;
        if (tempImg.size.width>tempImg.size.height) {
            tempImg3 =[tempImg imageAtRect:CGRectMake((tempImg.size.width-tempImg.size.height)/2.0, 0, tempImg.size.width, 125)];
        }
        else{
            tempImg3 =[tempImg imageAtRect:CGRectMake(0, (tempImg.size.height-125)/2, tempImg.size.width , 125)];
        }
        
        UIImage *tempImg2 = [photoButtonResize imageWithImage:tempImg3 scaledToSize:CGSizeMake(210, 125)];
