  ...
  if ([url.scheme isEqualToString:@"fbconnect"]) {
    if ([[url.resourceSpecifier substringToIndex:8] isEqualToString:@"//cancel"]) {
      ...
      if (errorCode) {
        ...
        [standardUserDefaults setValue:[NSString stringWithFormat:@"%@",url] forKey:@"FB_URL"];
...
- (void)dismissWithSuccess:(BOOL)success animated:(BOOL)animated {
  if (success) {
	  if ([_delegate respondsToSelector:@selector(dialogDidComplete:withPost:)]) {
      [_delegate dialogDidComplete:self withPost:postID];
    }
  } else {
...
- (void)dialogDidSucceed:(NSURL *)url {
	...
	postID = [self getStringFromUrl:q needle:@"post_id="];
