...
- (id)parseJsonResponse:(NSData*)data error:(NSError**)error {
	...
	NSString* responseString = [[[NSString alloc] initWithData:data 
								...
								autorelease];
	//NSLog(@"rrrrrreeeeeeeeesponce  %@",responseString);
...
//			return [NSDictionary dictionaryWithObject:@"true" forKey:@"result"];
//		}		
		/*appDelegate = (DemoAppAppDelegate *)[[UIApplication sharedApplication] delegate];
		NSDictionary *myResult= [[NSDictionary alloc] initWithDictionary:result];
		NSArray *mylist = [myResult objectForKey:@"posts"];
		if(mylist!=nil){
			if ([mylist isKindOfClass:[NSArray class]]) {
				NSMutableArray *tempArray = [[NSMutableArray alloc] initWithArray:mylist copyItems:YES];
				for(int i =0; i<[tempArray count];i++){
					NSMutableDictionary *postItem = [[NSMutableDictionary alloc] initWithDictionary:[tempArray objectAtIndex:i] copyItems:YES];
						
					if ([postItem objectForKey:@"attachment"]!= nil) {
						NSMutableDictionary *postAttachmentItem = [[NSMutableDictionary alloc] initWithDictionary:[postItem objectForKey:@"attachment"]copyItems:YES];
						NSString *str = [NSString stringWithFormat:@"%@",[postAttachmentItem objectForKey:@"fb_object_type"]];
						
						if([str isEqualToString:  @"album"]){
							if ([[postAttachmentItem objectForKey:@"media"] isKindOfClass:[NSArray class]]) {
								NSArray *postAttachmentMediaItem = [[NSArray alloc] initWithArray:[postAttachmentItem objectForKey:@"media"] copyItems:YES];
								
								for (int k=0; k<[postAttachmentMediaItem count]; k++) {
									if ([[postAttachmentMediaItem objectAtIndex:k] isKindOfClass:[NSMutableDictionary class]]) {
										
										NSLog(@"found characters  -- %@" ,[[postAttachmentMediaItem objectAtIndex:k] objectForKey:@"src"]);
										[appDelegate.photosArray addObject:[postAttachmentMediaItem objectAtIndex:k] ];
									}
		...
		//id result = [jsonParser objectWithString:responseString];
		//NSLog(@"aray------ %@",appDelegate.photosArray);
...
-(void)connectionDidFinishLoading:(NSURLConnection*)connection {
  [self handleResponseData:_responseText];
	NSLog(@"inside fbrequest.m  %@ :", _responseText);
  [_responseText release];
  _responseText = nil;
  [_connection release];
  _connection = nil;
